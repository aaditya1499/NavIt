...
class TouchImageView : ImageView {
    ...
    private fun sharedConstructing(context: Context) {
        ...
        mScaleDetector = ScaleGestureDetector(contextt, ScaleListener())
        matrixx = Matrix()
        m = FloatArray(9)
        imageMatrix = matrixx
        ...
        setOnTouchListener { v, event ->
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> if (mode == DRAG) {
                    ...
                    matrixx!!.postTranslate(fixTransX, fixTransY)
                    fixTrans()
                    last[curr.x] = curr.y
                }
    ...
    private inner class ScaleListener : SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            if (origWidth * saveScale <= viewWidth || origHeight * saveScale <= viewHeight) matrixx!!.postScale(
                mScaleFactor,
                mScaleFactor,
                viewWidth / 2.toFloat(),
                viewHeight / 2.toFloat()
            ) else matrixx!!.postScale(
                mScaleFactor,
                mScaleFactor,
                detector.focusX,
                detector.focusY
    ...
    fun fixTrans() {
        ...
        if (fixTransX != 0f || fixTransY != 0f) matrixx!!.postTranslate(fixTransX, fixTransY)
    }
    ...
    fun getFixTrans(
        ...
        return if (trans > maxTrans) -trans + maxTrans else 0.0f
    }
